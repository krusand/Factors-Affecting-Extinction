...
p5 <- ggplot(L_data,aes(Pairs, log(Time))) + geom_point() + ggtitle("Large Migrant") + ylab("Time in thousands [Y]") + xlab("") + theme(plot.title = element_text(hjust = 0.5)) + stat_smooth(method = "lm", level = 0)
p6 <- ggplot(S_data,aes(Pairs, log(Time))) + geom_point() + ggtitle("Large Resident")+ ylab("")  + xlab("") + theme(plot.title = element_text(hjust = 0.5))+ stat_smooth(method = "lm",level = 0)
...
m1 <- lm(formula = log(Time) ~ Pairs, data.out)
m2 <- lm(formula = log(Time) ~ Pairs + Size, data.out)
m3 <- lm(formula = log(Time) ~ Pairs + Size + Pairs*Size, data.out)
m4 <- lm(formula = log(Time) ~ Pairs + Size + Status, data.out)
m5 <- lm(formula = log(Time) ~ Pairs + Size + Status + Pairs*Size + Pairs * Status, data.out)
m6 <- lm(formula = log(Time) ~ Pairs + Size + Status + Pairs*Size + Pairs * Status + Size*Status + Pairs *Size*Status, data.out)
