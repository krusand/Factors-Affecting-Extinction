...
```{r}
lm_full_logtime <- lm(formula = log(Time) ~ Pairs + Size + Status + Pairs*Size + Pairs * Status + Size*Status + Pairs*Size*Status, data)
summary(lm_full_logtime)$adj.r.squared
```
...
```{r}
ckd <- cooks.distance(lm_full_logtime)
sub_ckd <- subset(ckd, ckd < 0.1)
index <- which(ckd > 0.1)

index
plot(ckd, ylim = c(0,0.25))
plot(sub_ckd, ylim = c(0,0.1))
```

????? non of them is significantly bigger than the others 

```{r}
data_ckd <- data[-c(index),]

lm_full_logtime_ckd <- lm(formula = log(Time) ~ Pairs + Size + Status + Pairs*Size + Pairs*Status + Size*Status + Pairs*Size*Status, data_ckd)
summary(lm_full_logtime_ckd)$adj.r.squared
